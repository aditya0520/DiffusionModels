...
class DDPMScheduler(nn.Module):
    ...
    def _get_variance(self, t):
        ...
        variance = current_beta_t * (1.0 - alpha_prod_t_prev) / (1.0 - alpha_prod_t) 
    ...
    def step(
        ...
        beta_prod_t = self.betas[t] 
        beta_prod_t_prev = self.betas[prev_t] 
        ...
        if self.prediction_type == 'epsilon':
            pred_original_sample = (sample - torch.sqrt(1 - current_alpha_t) * model_output) / torch.sqrt(current_alpha_t) 
        else:
        ...
        pred_original_sample_coeff = torch.sqrt(alpha_prod_t_prev) * beta_prod_t / (1 - alpha_prod_t)
